...
\begin{table}
	\begin{center}
		\begin{tabu}{ll}
			...
			\midrule
			\reg{a0}             & Exception selector register. \\
			...
			\creg{11} (\creg{sp})& Stack capability (\sandboxABI{}). \\
			\creg{12}            & Used with \asm{cjalr} as the destination register (\sandboxABI{}). \\
			\creg{13}            & Capability to on-stack arguments. \\
			\creg{11}--\creg{15} & Temporary (caller-save) registers.\\
			\creg{16}            & Exception pointer register. \\
			\creg{17} (\creg{ra})& Capability link register used with \asm{cjalr} (\sandboxABI{}).\\
			\creg{16}--\creg{25} & Saved (callee-save) registers. \\
			\creg{24} (\creg{fp})& Capability frame pointer (\sandboxABI{}).\\
			\creg{25} (\creg{bp})& Capability stack pointer (\sandboxABI{}).\\
			\creg{24}--\creg{31} & Not used by the compiler (\hybridABI{}).\\
			\creg{26}--\creg{31} & Not used by the compiler (\sandboxABI{}).\\
			\bottomrule
		\end{tabu}
